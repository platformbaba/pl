...
<div class="bloc" style="margin-top:0px;">
    <div class="title"><?=ucfirst($_GET['action'])?> Album Rights ( <?=urldecode($_GET['album_name']);?> )<div align="right"><span class='mandatory'>*&nbsp;</span>Mandatory Fields.</div></div>
    <div class="content">
	<form method="post" enctype="multipart/form-data" >
        <div class="clearfix">
			...
			<div class="clearfix">
				...
				<select  name="banner" id="banner"  >$bannerList_arr[8]->banner_name;
				<option value='' >--Select Banner Name--</option>
				<?php
				$i=0;
				foreach($bannerList_arr as $kBanner){
				    $bSelect = ($aContent['banner_id']==$bannerList_arr[$i]->banner_id)?'selected="selected"':'';
					echo '<option value='.$bannerList_arr[$i]->banner_id.' '.$bSelect.'>'.$bannerList_arr[$i]->banner_name.'</option>';
				$i++;
				}	
				?>
				</select>
