...
<div id="content">
	<div>
		<span class='FL'><h1><img src="<?php echo IMGPATH; ?>icons/region.png" alt=""  height="45px" width="45px" />Manage Regions</h1></span>
		<span class='FR MT40'>
			<?php if ($this->user->hasPrivilege(strtolower(MODULENAME)."_add")){ echo '<a href="'.SITEPATH.'region?action=add" title="Add New" class="button">Add New</a>'; } ?>
		</span>
		<br class='clear' />
	</div>
	<?php
    ...
    <div class="content">
		...
		<div class="clearfix">
			<div class="input FL50">
				<label for="input">Region Name:</label>
				<input type="text" id="srcRegion" name="srcRegion" value="<?php echo $aSearch['srcRegion']; ?>" />
			</div>
...
<div class="bloc">
    <div class="title">
        Regions Lists <b class='sCount'>(<?=$iTotalCount?>)</b>
    </div>
    <div class="content">
		...
		<table>
            ...
            <tbody>
				...
				?>
				<tr>
					<td><?php TOOLS::displayCheckBoxList( array( 'id'=>$showData->region_id, 'status'=>$showData->status, 'model'=>'region' ) ); ?></td>
					<td><?php echo $showData->region_id; ?></td>
					<td><?php echo $showData->region_name; ?></td>
                    <td class="actions">
						<?php TOOLS::displayActionHtml( $this, array( 'id'=>$showData->region_id, 'status'=>$showData->status, 'model'=>'region' ) ); ?>					
					</td>
                </tr>
				<?php
					} /* foreach end */
