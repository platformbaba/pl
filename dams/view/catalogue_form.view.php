...
<script>
	jQuery(document).ready(function(){
		...
		jQuery("#catalogueFrm").validationEngine({
...
<h1><img src="<?php echo IMGPATH; ?>icons/posts.png" alt="" />Catalogue</h1>
<?php
	/* To show errorr */
	if( !empty($aError) ){
		$aParam = array(
...
<div class="bloc">
    <div class="title"><?=ucfirst($_GET['action'])?> Catalogue <div align="right"><span class='mandatory'>*&nbsp;</span>Mandatory Fields.</div></div>
    <div class="content">
	<form method="post" enctype="multipart/form-data" id="catalogueFrm" name="catalogueFrm" >
		...
		<div class="clearfix">
			<div class="input FL50">
            	<label for="input2"> Catalogue Title : <span class='mandatory'>*</span></label>
				<input type="test" name='catalogueName' value="<?=$aContent['catalogueName']?>" class="validate[required]"/>
        	</div>
		</div>
		<div class="clearfix">
			<div class="input FL50">
			...
			<?php
			foreach($bannerList_arr as $kBanner){
		...
		<select name="territory_ex[]" id="territory_ex"  multiple="multiple" size="10" style="width:200px; height:200px;">	
		<?php
		...
		<input type="text" name="StartDate" class="datepicker validate[required]" id="StartDate" value="<?=$aContent['start_date']?>"/>
		</div>
		...
		<div class="clearfix">
			<div class="input FL50">
				...
				<input type="test" name='agreementId' value="<?=$aContent['agreementId']?>" />
        	</div>
			<div class="input FL50">
				<label for="input1">Agreement File : </label>
				<input type="file" id="agreementFile" name="agreementFile" value="<?=$aContent['agreementFile']?>"/>
				<input type="hidden" name="oFile" value="<?=$aContent['agreementFile']?>" />
		...
		<div class="clearfix">
			<div class="input textarea">
				...
				<textarea name="internalContactDetails" cols="5" rows="5" id="internalContactDetails"><?=$aContent['internalContactDetails']?></textarea>
        	</div>
		</div>
		<div class="clearfix">
			<div class="input textarea">
            	<label for="textarea2"> Contact Details : </label>
				<textarea name="contactDetails" cols="5" rows="5" id="contactDetails"><?=$aContent['contactDetails']?></textarea>
        	</div>
		</div>
		<div class="clearfix">
			<div class="input FL50">
